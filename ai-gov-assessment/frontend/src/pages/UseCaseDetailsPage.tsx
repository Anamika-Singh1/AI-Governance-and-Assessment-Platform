...
function SignalsGrid({ data }: { data: Record<string, unknown> }) {
  ...
  if (entries.length === 0) return <p className="text-sm text-slate-400 dark:text-slate-400">No data.</p>;
  return (
    <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-2">
      {entries.map(([key, value]) => (
        <div key={key} className="flex items-center justify-between gap-2 rounded-lg border border-slate-200 dark:border-slate-700 px-3 py-2 text-sm">
          <span className="text-slate-500 dark:text-slate-400">{labelize(key)}</span>
          <span className="font-medium text-slate-800 dark:text-slate-100">{formatFieldValue(value)}</span>
        </div>
      ))}
    </div>
  );
      ...
      <div className="mx-auto max-w-4xl space-y-6">
        ...
        <Card>
          <div className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <div className="text-xs uppercase tracking-wide text-slate-400 dark:text-slate-400">Latest assessment</div>
              {assessment ? (
                ...
                <div className="mt-1 text-sm text-slate-500 dark:text-slate-400">No assessment has been run against this use case yet.</div>
              )}
            </div>
          ...
          {assessment && (
            <div className="border-t border-slate-100 dark:border-slate-700 px-5 py-3 text-xs text-slate-400 dark:text-slate-400">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div>
              <div className="text-xs font-medium uppercase tracking-wide text-slate-400 dark:text-slate-400">Description</div>
              <p className="mt-1 text-sm text-slate-700 dark:text-slate-200">{useCase.description}</p>
            </div>
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div>
                <div className="text-xs font-medium uppercase tracking-wide text-slate-400 dark:text-slate-400">Purpose</div>
                <p className="mt-1 text-sm text-slate-700 dark:text-slate-200">{useCase.purpose}</p>
              </div>
              <div>
                <div className="text-xs font-medium uppercase tracking-wide text-slate-400 dark:text-slate-400">Data Used</div>
                <p className="mt-1 text-sm text-slate-700 dark:text-slate-200">{useCase.dataUsed}</p>
              </div>
              <div>
                <div className="text-xs font-medium uppercase tracking-wide text-slate-400 dark:text-slate-400">Intended Users</div>
                <p className="mt-1 text-sm text-slate-700 dark:text-slate-200">{useCase.intendedUsers}</p>
              </div>
              <div>
                <div className="text-xs font-medium uppercase tracking-wide text-slate-400 dark:text-slate-400">Affected Parties</div>
                <p className="mt-1 text-sm text-slate-700 dark:text-slate-200">{useCase.affectedParties}</p>
              </div>
            </div>
