...
export async function createAssessment(req: Request, res: Response) {
  ...
  const stored = await runAssessment(parsed.data.useCaseId);
  res.status(201).json(stored);
}

export async function listAssessmentsHandler(_req: Request, res: Response) {
...
export async function rerunAssessmentHandler(req: Request, res: Response) {
  const existing = await getAssessmentById(req.params.id);
  if (!existing) throw new AppError("Assessment not found.", 404, "NOT_FOUND");
  const rerun = await runAssessment(req.params.id);
